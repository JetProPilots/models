...
const getAddress = (user) => {
    if (!user || !user.employee || !user.employee.address)
        return null;
...
const getCategory = (user, categories) => {
    if (!user || !user.employee || !user.employee.skills)
        return '';
...
class MapMarker {
    ...
    constructor(user, map, places, categories, design, index = 0) {
        ...
        const place = places
            ...
            if (this.address && this.address.city && this.address.state) {
                return (this.address.city.toLowerCase() === p.city.toLowerCase() &&
        ...
        if (place) {
            ...
            console.log(`failed to get long/lat for ${this.address.city} ${this.address.state} ${this.address.zip}`);
            // fallback to NIIC
            this.lat = 41.1199397;
     ...
     */
    render({ id = this.index, map_id = this.map_id, address = '', pic = '', link = '', icon = _design.icons.pin, lat = jitter(this.lat), // gets populated by handler based on "place" by zip or address
    lng = jitter(this.lng), // gets populated by handler based on "place" by zip or address
    anim = '0', title = '', infoopen = '0', category = this.category, approved = '1', retina = '0', type = '0', did = '', other_data = '', sticky = '0', htmlTemplate = this.htmlTemplate, debug = false, escape = true, } = {}) {
        this.debug = !!debug;
        if (!htmlTemplate) {
